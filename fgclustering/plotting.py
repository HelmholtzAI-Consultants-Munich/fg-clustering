...
def _plot_global_feature_importance(p_value_of_features, save):
    ...
    plt.figure(figsize=(
    6.4, importance.shape[ 0 ] * 0.3))  # keep width default, change hight depending on the number of features
...
def _plot_heatmap(X, method, thr_pvalue, save):
    ...
    if n_features < 10:
        figure_size = n_features
    else:
        figure_size = n_features * 0.3
...
def _plot_boxplots(X, thr_pvalue, save, num_cols):
    ...
    plt.figure(figsize=(30, 10))  # 20, 12
    plt.tight_layout()
