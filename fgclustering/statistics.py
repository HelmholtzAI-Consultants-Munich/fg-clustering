...
def _sort_clusters_by_target(data_clustering_ranked, model_type):
    ...
    if model_type == 'classifier':
        ...
        data_clustering_ranked['target'] = data_clustering_ranked['target_orig'].astype('category').cat.codes
    
    means = data_clustering_ranked.groupby(['cluster']).mean().sort_values(by='target',ascending=True)
    ...
    data_clustering_ranked['cluster'] = pd.Categorical(data_clustering_ranked['cluster'], sorted(data_clustering_ranked['cluster'].unique()))
...
def calculate_global_feature_importance(X, y, cluster_labels, model_type):
    ...
    for feature in X.columns:
        assert pd.api.types.is_categorical_dtype(X[feature]) or pd.api.types.is_numeric_dtype(X[feature]), f"Feature {feature} has to be of type category of numeric!"
...
def _calculate_p_value_categorical(X_feature_cluster, X_feature, cluster, cluster_size, bootstraps):
    ...
    X_feature_cluster_impurity = compute_balanced_average_impurity(X_feature_cluster, cluster_label, rescaling_factor=rescaling_factor)

    bootstrapped_impurity = list()
    for b in range(bootstraps):
        bootstrapped_X_feature = resample(X_feature, replace = False, n_samples = cluster_size)
        bootstrapped_impurity.append(compute_balanced_average_impurity(bootstrapped_X_feature, cluster_label, rescaling_factor=rescaling_factor))
...
def calculate_local_feature_importance(data_clustering_ranked, bootstraps_p_value):
    ...
    for feature in data_clustering_ranked.columns:
        assert pd.api.types.is_categorical_dtype(data_clustering_ranked[feature]) or pd.api.types.is_numeric_dtype(data_clustering_ranked[feature]), f"Feature {feature} has dytpye {data_clustering_ranked[feature].dtype} but has to be of type category or numeric!"
